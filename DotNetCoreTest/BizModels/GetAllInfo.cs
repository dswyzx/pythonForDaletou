...
namespace DotNetCoreTest
{
    ...
    {
        private static WriteHelper latestMovieList = new WriteHelper(Path.Combine(ConstsConf.WWWRootPath, ConstsConf.txtName));
            ...
            if (lstDivInfo != null)
            {
                ...
                .ForEach(tr =>
                {
                    ...
                    if (info != null && info.DLTNum != null)
                        latestMovieList.AddToMovieDic(info);
                });
            }
        }
        ...
        public static List<DLTInfo> GetAllDLTNUM()
        {
            ...
            using (var stream = new FileStream(path, FileMode.OpenOrCreate))
            {
                try
                {
                    ...
                    {
                        var lstMovie = serializer.Deserialize<List<DLTInfo>>(reader);
                        // var res = serializer.Deserialize<DLTInfo>(reader);
                        foreach (var item in lstMovie.Distinct().ToList())
                        {
                            list.Add(item);
                        }
                    }
